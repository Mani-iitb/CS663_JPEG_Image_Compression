...
for z = 1:20
    ...
    for y = 5:10:85
        ...
        figure(y);
        imshow(inv_dcted, []);
        save_image = uint8(inv_dcted);
        imwrite(save_image, "../grey_reconstructed/Reconstructed_"+num2str(z)+"_"+num2str(y)+".jpg");
...
for p = 1:20
    ...
    grid on;
    Q=5:10:85;
    for a = 1:length(bppValues)
        % Add a label near each point
        text(bppValues(a), rmseValues(a), sprintf('Q=%d', Q(a)), ...
