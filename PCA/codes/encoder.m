...
image = Y;
restructred = [];
for i=1:8:size(image, 1)-7
    for j = 1:8:size(image, 2)-7
        restructred = [restructred reshape(image(i:i+7, j:j+7),[],1)];
    end
end

meanImage = mean(restructred, 2);
centeredImage = restructred - meanImage;
covMatrix = centeredImage * centeredImage';
[eigenVectors, eigenValues] = eig(covMatrix);
eigenValues = diag(eigenValues);
[~, indices] = sort(eigenValues, 'descend');  
eigenVectors = eigenVectors(:, indices);
k = 32;
topEigenVectors = eigenVectors(:, 1:k);
Y = topEigenVectors' * centeredImage;
%% 
arr=[];
...
flat_eigenvectors = reshape(topEigenVectors, [], 1);


%% 
fileID = fopen("comp.mv", "w");
fwrite(fileID, orig_row, 'uint32');
fwrite(fileID, orig_col, 'uint32');
...
fwrite(fileID, size(meanImage,1),"uint8");
fwrite(fileID, meanImage, "uint8");
fwrite(fileID, size(flat_eigenvectors,1),"uint32");
fwrite(fileID, flat_eigenvectors, "float");
